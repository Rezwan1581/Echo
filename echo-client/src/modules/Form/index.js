...
const Form = ({
    ...
    return (
        <div className="bg-white w-[400px] h-[600px] shadow-lg rounded-lg flex flex-col justify-center items-center">
            ...
            {isSignInPage ? (
                <div className="text-m font-light mb-14">Sign in to explore</div>
            ) : (
                <div className="text-m font-light mb-14">Sign up to get started!!!</div>
            )}


            <form className='flex flex-col items-center w-full' onSubmit={() => console.log('Submitted')}>
                {!isSignInPage && <Input label="First name" name="name" placeholder="example: Rezwan" className="mb-4" value={data.firstName}
                    onChange={(e) => setdata({ ...data, firstName: e.target.value })} />}
                {!isSignInPage && <Input label="Last name" name="name" placeholder="example: Ahmed" className="mb-4" value={data.lastName}
                    onChange={(e) => setdata({ ...data, lastName: e.target.value })} />}
                <Input label="Email" type="email" name="email" placeholder="example: rezwan.ahmed@email.com" className="mb-6" value={data.email}
